...
<template>
  <div class="basePostClass" @click="handlePostClick(post._id)">
    <div
      v-if="isRepost"
      class="flex items-center text-[0.813rem] font-bold text-[#71767A] ml-11 pt-1 -mb-1"
    >
      <RepostIcon class="w-4 h-4 mr-2" />
      <span>You reposted</span>
    </div>

    <div :class="['flex', isRepost ? 'px-4 pb-1 gap-3' : 'px-4 py-1 gap-3']">
      <img
        :src="userAvatar"
      ...
      <div class="flex-1">
        <header class="leading-5 flex items-center gap-2">
          <a
            href="#"
        ...
        <div>
          <p :class="{ 'text-sm text-white': isRepost }">{{ postContent }}</p>
            ...
            <div class="flex-1 group flex items-center gap-px">
              <div
                :class="[
                  'icon-bg',
                  isReposted
                    ? 'text-[#00BA7C] group-hover:bg-[#00ba7c1a]'
                    : 'group-hover:bg-[#00ba7c1a] group-hover:text-[#00ba7c]',
                ]"
                @click="(event) => handleRepost(event, post._id)"
              >
                <component :is="isReposted ? FocusRepostIcon : RepostIcon" />
              </div>
              <span
                :class="[
